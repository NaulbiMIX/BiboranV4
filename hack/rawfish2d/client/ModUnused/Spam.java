...
public class Spam extends Module{
	...
	public static List<String> delayed_list = new ArrayList<String>();
	public static int amount = 50000;
	
	public Spam() {
		super("Spam-Store", 0, ModuleType.MISC);
		setDescription("–‡Á‰‡˜‡ ‰ÂÌÂ„ ÔÓ 50Í (Á‡ÔËÒ¸ ‚ Ó˜ÂÂ‰¸)");
		instance = this;
	}
	...
	public void onUpdate() {
		...
		if (System.nanoTime() / 1000000L - prevMs >= var1) {
			...
			if(delayed_list.size() > 0) {
				String name = delayed_list.get(0);
				delayed_list.remove(0);
				Pay(name, 50000);
			}
			//SPAM();
		}
	...
	public void onChatMessage(String str) {
		...
		if(str.contains(msg)) {
			...
			if(n1 + 4 < n2 && n1 < str.length() && n2 < str.length()) {
				...
				//if(!name.equalsIgnoreCase("NewGod911") && !name.equalsIgnoreCase("Kevin052") && !name.equalsIgnoreCase("ddos1337"))
	...
	*/
	public static void Pay(String name, int amount) {
		mc.thePlayer.sendChatMessage("/pay " + name + " " + amount);
	}
