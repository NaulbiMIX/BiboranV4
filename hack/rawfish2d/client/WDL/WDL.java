...
public class WDL
{
	...
	public static void loadWorld()
	{
		...
		INetworkManager newNM = tp.sendQueue.getNetManager(); // tp.sendQueue.getNetManager()
	...
	{
		Block block = MiscUtils.getBlock(te.xCoord, te.yCoord, te.zCoord); 
	...
	public static void savePlayer(NBTTagCompound playerNBT)
	{
		...
		try
		{
			File playersDirectory = new File("saves/" + saveHandler.getWorldDirectoryName(), "players");
			String name = "";
			if(Client.getInstance().mc.session == null) {
				name = "player";
			}
			else {
				name = Client.getInstance().mc.session.username;
			}
			
			File playerFile = new File(playersDirectory, name + ".dat.tmp");
			File playerFileOld = new File(playersDirectory, name + ".dat");
			CompressedStreamTools.write(playerNBT, new File(playersDirectory, name + ".dat.tmp"));
		...
		{
			e.printStackTrace();
			chatDebug("Couldn't save the player!");
	...
	public static void saveWorldInfo(NBTTagCompound worldInfoNBT)
	{
		...
		File saveDirectory = new File("saves/" + saveHandler.getWorldDirectoryName()); //idk
		...
		catch (Exception e)
		{
			...
			e.printStackTrace();
			chatDebug("Couldn't save the world metadata!");
		}
	...
	public static void applyOverridesToPlayer(NBTTagCompound playerNBT)
	{
		...
		if (playerPos.equals("xyz"))
		{
			...
			((NBTTagDouble)pos.tagAt(0)).data = (x + 0.5D);
			((NBTTagDouble)pos.tagAt(1)).data = ((double)y + 0.621D);
			((NBTTagDouble)pos.tagAt(2)).data = (x + 0.5D);
			...
			//motion.appendTag(new NBTTagDouble(0.0D));
			((NBTTagDouble)motion.tagAt(0)).data = 0.0D;
			((NBTTagDouble)motion.tagAt(1)).data = -0.001D;
			((NBTTagDouble)motion.tagAt(2)).data = 0.0D;
			...
			//rotation.appendTag(new NBTTagFloat(0.0f));
			((NBTTagDouble)motion.tagAt(0)).data = 0.0F;
			((NBTTagDouble)motion.tagAt(1)).data = 0.0F;
		}
	}
	...
	public static String getServerName()
	{
		try
		{
			if (mc.getServerData() != null) // getServerData
			{
				String name = mc.getServerData().serverName;
				/*
				if(name.equals(I18n.format("selectServer.defaultName"))) //idk
				{
	...
	*/
	...
	public static void chatMsg(String msg)
	{
		...
		mc.ingameGUI.getChatGUI().printChatMessage(new String("\u00A7c[WorldDL]\u00A76 " + msg));
	}
	...
	public static void chatDebug(String msg)
	{
		if (!WDL.DEBUG)
			return;
		// System.out.println( "WorldDownloader: " + msg ); // Just for debugging!
		mc.ingameGUI.getChatGUI().printChatMessage(new String("\u00A72[WorldDL]\u00A76 " + msg));
	}
	...
	{
		// System.out.println( "WorldDownloader: " + msg ); // Just for debugging!
		mc.ingameGUI.getChatGUI().printChatMessage(new String("\u00A72[WorldDL]\u00A74 " + msg));
	}
