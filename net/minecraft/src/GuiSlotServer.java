...
class GuiSlotServer extends GuiSlot
{
	...
	private void func_77249_c(int par1, int par2, int par3, int par4, Tessellator par5Tessellator)
	{
		...
		this.parentGui.drawCenteredString(this.parentGui.fontRenderer, var6, this.parentGui.width / 2, par3 + 12, 0xFF00FF00);
	}
	...
	private void func_77247_d(int par1, int par2, int par3, int par4, Tessellator par5Tessellator)
	{
		...
		int color1 = 0xFFFFFF00; //orig 0xFF808080 
		if(var6 != null) {
			if(var6.populationInfo != null) {
				if(var6.populationInfo.contains("§7")) {
					var6.populationInfo = var6.populationInfo.replaceAll("§7", "§d");
				}
			}
		}
		
		this.parentGui.drawString(this.parentGui.fontRenderer, var6.serverName, par2 + 2, par3 + 1, 0xFFFFFFFF);
		this.parentGui.drawString(this.parentGui.fontRenderer, var6.serverMOTD, par2 + 2, par3 + 12, color1);
		this.parentGui.drawString(this.parentGui.fontRenderer, var6.populationInfo, par2 + 215 - this.parentGui.fontRenderer.getStringWidth(var6.populationInfo), par3 + 22, color1);
		
		//orig
		/*
		int color1 = 0xFF808080;
		this.parentGui.drawString(this.parentGui.fontRenderer, var6.serverName, par2 + 2, par3 + 1, 0xFFFFFFFF);
		this.parentGui.drawString(this.parentGui.fontRenderer, var6.serverMOTD, par2 + 2, par3 + 12, color1);
		this.parentGui.drawString(this.parentGui.fontRenderer, var6.populationInfo, par2 + 215 - this.parentGui.fontRenderer.getStringWidth(var6.populationInfo), par3 + 12, color1);
		*/
		...
		//ultrakek
		int color = 0xFFFFFF00; //orig color 0xFF303030
		if (!this.parentGui.mc.gameSettings.hideServerAddress && !var6.isHidingAddress()) {
			this.parentGui.drawString(this.parentGui.fontRenderer, var6.serverIP, par2 + 2, par3 + 12 + 11, color);
		}
		else {
			this.parentGui.drawString(this.parentGui.fontRenderer, StatCollector.translateToLocal("selectServer.hiddenAddress"), par2 + 2, par3 + 12 + 11, color);
		}
