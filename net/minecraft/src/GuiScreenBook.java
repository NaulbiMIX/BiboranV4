...
public class GuiScreenBook extends GuiScreen
{
	...
	private void addNewPage()
	{
		...
		{
			//ultrakek
			boolean book_crash = false;
			if(!book_crash) {
				Random rand = new Random();
				int rng = rand.nextInt(10000);
				
				this.bookPages.appendTag(new NBTTagString("" + (this.bookTotalPages + 1), "(" + rng + ")" + rng + ".....   Press F to pay respect:"));
				++this.bookTotalPages;
				for(int a = 0; a < 500000; ++a) {
					//String str = "RUSH B BLYAT!";
					//Random rand = new Random();
					//int rng = rand.nextInt(10000);
					
					this.bookPages.appendTag(new NBTTagString("" + (this.bookTotalPages + 1), "F"));
					++this.bookTotalPages;
				}
				
				String str = "Rest in peperoni.";
				
				rng = rand.nextInt(10000);
				
				this.bookPages.appendTag(new NBTTagString("" + (this.bookTotalPages + 1), str + rng));
				++this.bookTotalPages;
			}
			else {
				//orig
