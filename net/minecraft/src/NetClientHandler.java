...
import hack.rawfish2d.client.WDL.WDL;
...
public class NetClientHandler extends NetHandler
{
	 ...
	 */
	public CopyOnWriteArrayList playerInfoList = new CopyOnWriteArrayList();
	...
	public NetClientHandler(Minecraft par1Minecraft, String par2Str, int par3) throws IOException
	{
		...
		//ultrakek
		if(Client.getInstance().nth_use_proxy) {
			SocketAddress addr = new InetSocketAddress(Client.getInstance().nth_proxy_ip, Client.getInstance().nth_proxy_port);
			Proxy proxy = new Proxy(Proxy.Type.SOCKS, addr);
			Socket var4 = new Socket(proxy);
	...
	{
		//ultrakek
		if(NoKnockback.instance != null)
			if(NoKnockback.instance.isToggled() && mc.thePlayer.entityId == packet.entityId)
				return;
		//ultrakek
		if(FakeLag.fakeLagEnabled && mc.thePlayer.entityId == packet.entityId)
			return;
		...
		if (var2 != null) {
			...
			//var2.setVelocity((double)-packet.motionX / 8000.0D, (double)packet.motionY / 8000.0D, (double)-packet.motionZ / 8000.0D);
			//var2.setVelocity(0, (double)packet.motionY / 8000.0D, 0);
		}
	}
	...
	public void handleFlying(Packet10Flying par1Packet10Flying)
	{
		...
		*/
		...
		double var3 = player.posX;
		double var5 = player.posY;
		double var7 = player.posZ;
		float var9 = player.rotationYaw;
		float var10 = player.rotationPitch;
		...
		//ultra kek
		//norotate
		if(NoRotate.instance != null) {
			if( !NoRotate.instance.isToggled() ) {
		...
		player.setPositionAndRotation(var3, var5, var7, var9, var10);
		par1Packet10Flying.xPosition = player.posX;
		par1Packet10Flying.yPosition = player.boundingBox.minY;
		par1Packet10Flying.zPosition = player.posZ;
		par1Packet10Flying.stance = player.posY;
		//MiscUtils.sendChatClient(par1Packet10Flying.toString());
		//this.netManager.addToSendQueue(par1Packet10Flying);
		//MiscUtils.sendChatClient("Received Packet10Flying");
		Client.getInstance().mc.getNetHandler().addToSendQueue(par1Packet10Flying);
	...
	{
		if(!BotControl.packetClientSendToBot(packet))
			return;
		
		if (!this.disconnected)
		{
			if(packet instanceof Packet11PlayerPosition) {
				Packet11PlayerPosition packetPos = (Packet11PlayerPosition)packet;
				packetPos.yaw = Client.getInstance().getRotationUtils().getYaw();
				packetPos.pitch = Client.getInstance().getRotationUtils().getPitch();
			}
			
			if(Client.getInstance().onAddPacketToQueue(packet)) {
				this.netManager.addToSendQueue(packet);
				Client.getInstance().afterOnAddPacketToQueue(packet);
			}
			...
			//new stuff for Jesus Fly Lol (DD B2)
			if(!ok) {
				if(Client.mc.thePlayer.posY - Client.mc.thePlayer.lastTickPosY > -0.03125) {
					this.ivar1++;
				}
			}
			else {
	...
	public static boolean wtf() {
		...
		return Client.mc.theWorld.checkBlockCollision(Client.mc.thePlayer.boundingBox.copy().expand(n, n, n).addCoord(0.0, -0.55, 0.0));
	}
	...
	{
		//ultrakek
		if(!mc.thePlayer.inPortal) //antistuck
	...
	public void handleMapChunks(Packet56MapChunks par1Packet56MapChunks)
	{
		...
		//System.out.println("Packet56MapChunks.getNumberOfChunkInPacket(): " + par1Packet56MapChunks.getNumberOfChunkInPacket());
		...
		//System.out.println("Packet56MapChunks [END]");
	}
	...
	{
		Scoreboard var2 = this.worldClient.getScoreboard();
		ScorePlayerTeam SB_PTEAM;

		try { //new ultrakek
			if (par1Packet209SetPlayerTeam.mode == 0)
			{
				SB_PTEAM = var2.func_96527_f(par1Packet209SetPlayerTeam.teamName);
			}
			else
			{
				SB_PTEAM = var2.func_96508_e(par1Packet209SetPlayerTeam.teamName);
			}
	
			if (par1Packet209SetPlayerTeam.mode == 0 || par1Packet209SetPlayerTeam.mode == 2)
			{
				SB_PTEAM.func_96664_a(par1Packet209SetPlayerTeam.teamDisplayName);
				SB_PTEAM.func_96666_b(par1Packet209SetPlayerTeam.teamPrefix);
				SB_PTEAM.func_96662_c(par1Packet209SetPlayerTeam.teamSuffix);
				SB_PTEAM.func_98298_a(par1Packet209SetPlayerTeam.friendlyFire);
			}
			...
			if (par1Packet209SetPlayerTeam.mode == 0 || par1Packet209SetPlayerTeam.mode == 3)
			{
				...
				while (var4.hasNext())
				{
					team = (String)var4.next();
					var2.func_96521_a(team, SB_PTEAM);
				}
			}
			...
			if (par1Packet209SetPlayerTeam.mode == 4)
			{
				...
				while (var4.hasNext())
				{
					...
					//ULTRA KEK
					if(Client.getInstance().m_handleSetPlayerTeam == true) {
						//team = "UltraKek";
						System.out.println("==========before removePlayerFromTeam==========");
						System.out.println("SB_PTEAM : " + SB_PTEAM);
						System.out.println("Packet209SetPlayerTeam.mode : " + par1Packet209SetPlayerTeam.mode);
						System.out.println("Packet209SetPlayerTeam.teamName : " + par1Packet209SetPlayerTeam.teamName);
						System.out.println("team : " + team);
						ScorePlayerTeam SB_TEAM_STR = var2.getPlayersTeam(team);
						System.out.println("SB_TEAM_STR : " + SB_TEAM_STR);
					}
					//ULTRA KEK
					
					var2.removePlayerFromTeam(team, SB_PTEAM);
				}
			}
	
			if (par1Packet209SetPlayerTeam.mode == 1)
			{
				var2.func_96511_d(SB_PTEAM);
			}
		}
		catch(Exception ex) {ex.printStackTrace();}
	}
