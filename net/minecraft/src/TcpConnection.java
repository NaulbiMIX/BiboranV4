...
public class TcpConnection implements INetworkManager
{
	...
	public NetHandler theNetHandler;
	...
	public TcpConnection(ILogAgent par1ILogAgent, Socket par2Socket, String par3Str, NetHandler par4NetHandler, PrivateKey par5PrivateKey) throws IOException
	{
		...
		//System.out.println("ip: " + GuiConnecting.IP);
		//System.out.println("port: " + GuiConnecting.PORTINT);
		
		//ultrakek
		if(Client.getInstance().nth_use_proxy) {
			//socks4 enable
			ProxyManager.enableSocks4(networkSocket);
			this.networkSocket.connect( new InetSocketAddress(GuiConnecting.IP, GuiConnecting.PORTINT));
		}
		
		//this.networkSocket.connect( new InetSocketAddress(GuiConnecting.IP, 25565));
		//if(Client.nth_use_proxy)
		//	this.networkSocket.connect( new InetSocketAddress(Client.nth_proxy_ip, Client.nth_proxy_port));
	...
	public void addToSendQueue(Packet par1Packet)
	{
		if (!this.isServerTerminating)
		{
			...
			synchronized (this.sendQueueLock)
			{
				...
				//System.out.println("SEND #" + par1Packet.getPacketId() + " : " + par1Packet.toString());
		...
		try
		{
			...
			if (this.field_74468_e == 0 || !this.dataPackets.isEmpty() && System.currentTimeMillis() - ((Packet)this.dataPackets.get(0)).creationTimeMillis >= (long)this.field_74468_e)
			{
				...
				if (var2 != null) {
					...
					//ultrakek
					//ConfigManager.savePacketName("SEND #" + var2.getPacketId() + " : " + var2.toString());
					//System.out.println("SEND #" + var2.getPacketId() + " : " + var2.toString());
	...
	private boolean readPacket()
	{
		...
		try
		{
			...
			if (var2 != null) { 
				//ultrakek
				Client.getInstance().onPacket(var2);
				
				if(Client.getInstance().DontProcessPacket(var2) == true) //do not process packet
					return false;
				
				//ULTRA KEK 
				/*
				if(Client.readPackets == true && Client.readLimit < Client.readLimitMax) {
					System.out.println("#" + Client.readLimit + " READ : " + var2.toString());
					Client.readLimit++;
				}
				*/
				//ULTRA KEK 
				...
				java.awt.EventQueue.invokeLater(new Runnable() {
				      public void run() {
				           ConfigManager.savePacketName("READ #" + var2.getPacketId() + " : " + var2.toString());
				      }
				});
				*/
				//System.out.println("READ #" + var2.getPacketId() + " : " + var2.toString());
