...
public abstract class Entity
{
	...
	public static double some_entity_var;
	...
	public void setAngles(float par1, float par2)
	{
		...
		//ULTRA KEK
		if (!CameraRotation.instance.isToggled()) {
	...
	{
		/*
		if(this == Client.mc.thePlayer) {
			Exception ex = new Exception();
			ex.printStackTrace();
			MiscUtils.sendChatClient("par1: " + par1);
			MiscUtils.sendChatClient("par3: " + par3);
			MiscUtils.sendChatClient("par5: " + par5);
		}
		*/
		//ULTRA KEK
		/*
		if (Client.mc.thePlayer != null && PacketStep.instance.isToggled()) {
			if (!Client.mc.thePlayer.movementInput.jump && Client.mc.thePlayer.isCollidedHorizontally && Client.mc.thePlayer.isCollidedVertically) {
				Client.mc.thePlayer.stepHeight = 1.35f;
				for (int i = 0; i < 10; ++i) {
					Client.mc.thePlayer.sendQueue.addToSendQueue(new Packet11PlayerPosition(Client.mc.thePlayer.posX, Client.mc.thePlayer.boundingBox.minY + 1.3, Client.mc.thePlayer.posY + 1.3, Client.mc.thePlayer.posZ, Client.mc.thePlayer.onGround));
				}
			}
			else {
				Client.mc.thePlayer.stepHeight = 0.6f;
			}
		}
		*/
		//ULTRA KEK
		...
		else
		{
			...
			//	Client.mc.gameSettings.keyBindJump.pressed = false;
			
			//ULTRA KEK
			//if (var20 || (Scaffold.instance.isToggled() || SafeWalk.instance.isToggled() || Tower.instance.isToggled())) {
			//if (var20 || (SafeWalk.instance.isToggled() || Tower.instance.isToggled())) {
			if (var20 || Scaffold.instance.isToggled()) {
				
			//if (var20 || Tower.instance.isToggled()) {
			//orig
			...
			//if (var20) {
				...
				for (; par5 != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox.getOffsetBoundingBox(0.0D, -1.0D, par5)).isEmpty(); var17 = par5)
				{
					if (par5 < var21 && par5 >= -var21)
					{
						//ultrakek
						//auto jump
						//Client.mc.gameSettings.keyBindJump.pressed = true;
			...
			if (this.stepHeight > 0.0F && var34 && (var20 || this.ySize < 0.05F) && (var13 != par1 || var17 != par5))
			{
				...
				if(Step.instance != null) {
					if (Step.instance.isToggled()) {
						for (double n5 = this.stepHeight * 0.1; n5 <= this.stepHeight; n5 += this.stepHeight * 0.1) {
							double n6 = par1;
							double n7 = par3;
							double n8 = par5;
							par1 = var13;
							par3 = n5;
							par5 = var17;
							AxisAlignedBB copy2 = boundingBox.copy();
							boundingBox.setBB(var19);
							List collidingBoundingBoxes2 = worldObj.getCollidingBoundingBoxes(this, boundingBox.addCoord(var13, par3, var17));
							for (int l = 0; l < collidingBoundingBoxes2.size(); ++l) {
								par3 = ((AxisAlignedBB) collidingBoundingBoxes2.get(l)).calculateYOffset(boundingBox, par3);
							}
							boundingBox.offset(0.0, par3, 0.0);
							...
							for (int l = 0; l < collidingBoundingBoxes2.size(); ++l) {
								par1 = ((AxisAlignedBB) collidingBoundingBoxes2.get(l)).calculateXOffset(boundingBox, par1);
							}
							boundingBox.offset(par1, 0.0, 0.0);
							...
							for (int l = 0; l < collidingBoundingBoxes2.size(); ++l) {
								par5 = ((AxisAlignedBB) collidingBoundingBoxes2.get(l)).calculateZOffset(boundingBox, par5);
							}
							boundingBox.offset(0.0, 0.0, par5);
							...
							if (n6 * n6 + n8 * n8 >= par1 * par1 + par5 * par5) {
								par1 = n6;
								par3 = n7;
								par5 = n8;
								boundingBox.setBB(copy2);
							}
							if (n6 * n6 + n8 * n8 < par1 * par1 + par5 * par5) {
								some_entity_var = stepHeight + par3;
								break;
							}
							some_entity_var = 0.0;
						}
					...
					//if step is disabled
		                ...
		                //ULTRA KEK
						if (var36 * var36 + var25 * var25 < par1 * par1 + par5 * par5) {
							some_entity_var = stepHeight + par3;
						}
						some_entity_var = 0.0;
						//ULTRA KEK
				...
				//if step is disabled or doesn't exist
				//ULTRA KEK
				else {
	                var36 = par1;
	                var25 = par3;
