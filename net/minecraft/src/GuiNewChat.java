...
public class GuiNewChat extends Gui
{
	...
	public boolean field_73769_e = false;
	...
	{
		//ultrakek
		//this.mc.gameSettings.chatHeightFocused = 2.5f;
			...
			if (var5 > 0)
			{
				...
				if (var3)
				{
					...
					if (var18 != var11)
					{
						...
						//drawRect(2, -var19, 1, -var19 - var13, 0xFFCCCCCC + (var14 << 24));
					}
				}
	...
	{
		//ultrakek
		Client.getInstance().onChatMessage(par1Str);
		//EventRecieveMessage event = (EventRecieveMessage) Client.getInstance().getEventManager().call(new EventRecieveMessage(this, par1Str));
		//par1Str = event.getMessage();
	...
	private void func_96129_a(String par1Str, int par2, int par3, boolean par4)
	{
		...
		//ultrakek new
		if(ShopDupe.instance != null) {
			if(ShopDupe.instance.isToggled())
				if(		par1Str.contains("§6Вы купили/продали предмет!") ||
						par1Str.startsWith("§6Установлен игровой режим") ||
						par1Str.startsWith("Your game mode has been updated") || 
						par1Str.startsWith("Ваш"))
					return;
		}
		//ultrakek new
		...
		int max_lines = 500;
		/*
		String str = par1Str.toLowerCase();
		String msg2 = GuiScreenGame.msg2.toLowerCase();;
		
		String msg1 = "§6[Я§6 ->";
		msg1 = msg1.toLowerCase();
		
		
		if(str.contains(GuiScreenGame.msg.toLowerCase()) || (str.contains(msg1) && str.contains(msg2)))
			return;
		*/
		/*
		String str = par1Str.toLowerCase();
		if(str.contains(GuiScreenGame.hide)) {
			return;
		}
		*/
		//ultrakek
		...
		while (this.field_96134_d.size() > max_lines)
		{
			//ultrakek
			try {
