...
public abstract class BlockFluid extends Block
{
	...
	public AxisAlignedBB getCollisionBoundingBoxFromPool(World par1World, int par2, int par3, int par4)
	{
		...
		if(Jesus.instance != null) {
			if (Client.getInstance().mc.thePlayer == null)
				return null;
			
			boolean jesus_toggled = Jesus.instance.isToggled();
			boolean not_legit = !Jesus.legit.getValue();
			boolean not_liquid_under = !Jesus.isLiquidUnderPlayer2();
			boolean not_sneaking = !Client.getInstance().mc.thePlayer.isSneaking();
			
			if(jesus_toggled && not_legit && not_liquid_under && not_sneaking)
				return AxisAlignedBB.getBoundingBox(par2 + minX, par3 + minY, par4 + minZ, par2 + maxX, par3 + maxY - 0.0001, par4 + maxZ);
		}
		//ultrakek
