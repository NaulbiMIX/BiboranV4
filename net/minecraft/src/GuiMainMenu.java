...
public class GuiMainMenu extends GuiScreen
{
	 ...
	 */
	public static int viewportTexture;
	...
	public void drawScreen(int par1, int par2, float par3)
	{
		...
		if(mc.currentScreen instanceof GuiMainMenu){
			if(!(mc.currentScreen instanceof UGuiMainMenu)){
				mc.displayGuiScreen(new UGuiMainMenu());
			}else{
		...
		*/
		...
		//этот код показывает главное меню если мы выходим с сервера
		if(mc.currentScreen instanceof GuiMainMenu){
			if(!(mc.currentScreen instanceof UGuiMainMenu)){
				mc.displayGuiScreen(new UGuiMainMenu());
				return;
